...
class NVT_Thread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.log_signal.emit(f"Erro durante NVT equilibration: {e}")
...
class PreparoEq(QWidget):
    ...
    def on_NVT_finished(self, success):
        if success:
            ...
            # Exibindo o gráfico da variação de temperatura:
            self.log("Displaying temperature variation graph...")
            if shutil.which("xmgrace"):
                subprocess.run("xmgrace temperature.xvg -autoscale xy", shell=True, cwd=self.run_folder)
            self.log("\nNVT Equilibration completed successfully!\n")
        else:
            self.log("Error during execution of NVT Equilibration.")
    ...
    def start_equilibration(self, nome_proteina: str, nome_ligante: str):
        try:
            ...
            try:
                ...
                self.nvt_thread.log_signal.connect(self.log)
