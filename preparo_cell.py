...
class MinimizationThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.log_signal.emit(f"Erro durante minimização: {e}")
            self.finished_signal.emit(False)


class PreparoCell(QWidget):
    def __init__(self, codyn_dir: str, config_data: dict, status_cell: QTextEdit, run_folder: str):
        super().__init__()
    ...
    def log(self, message: str):
        ...
        self.status_cell.append(f"[{ts}] {message}")
        self.status_cell.verticalScrollBar().setValue(
            self.status_cell.verticalScrollBar().maximum()
        )
    ...
    def start_preparation(self, nome_proteina: str, nome_ligante: str):
        try:
            ...
            with open(self.config_data, 'r') as f:
                lines = f.readlines()
                conc_ions = float(lines[5].strip())  # Concentração de íons
            ...
            try:
                ...
                self.min_thread.log_signal.connect(self.log)
